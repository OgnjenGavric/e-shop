...
const SelectColor: React.FC<SelectColorProps> = ({
  ...
  return (
    <div className='grid grid-cols-1 oweeflow-y-auto border-b-[1.2px] border-slate-22 items-center p-2 '>
      <div className='flex flex-row gap-2 items-center h-[60px]'>
        <input id={item.color} type='checkbox' checked={isSelected} onChange={handleCheck} className='cursor-pointer' />
        <label htmlFor={item.color} className='cursor-pointer'>{item.color}</label>
      </div>
      <>
      {isSelected && !file && (
